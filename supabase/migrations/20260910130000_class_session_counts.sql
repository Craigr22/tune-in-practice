-- What a student's class has done, so they can see where they stand.
--
-- A student can read only their own row in `students` and only their own
-- `practice_logs`, which is right — they have no business reading a
-- classmate's practice. So there is no query the page could run for itself.
-- This returns the one thing the board needs and nothing else: a name to show
-- and a count, for the classes the caller is actually in.
--
-- Surnames never leave the server. Names come back as "Payal M." — enough for
-- a child to find themselves in a list of six, and not a roster of full names
-- handed to every pupil.
--
-- The count is sessions finished, all-time. Not a run: nothing a student has
-- done is ever taken away from them by missing a week.
